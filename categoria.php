...
if (!isset($_GET['id']) or !is_numeric($_GET['id'])) {
    header("Location: index.php?status=erro_categoria");
    echo '<script>location.href="index.php?status=erro_categoria"</script>';
    exit;
}
$categoria = Categoria::getCategoria($_GET['id']);

//VALIDA SE A CATEGORIA EXISTE OU ESTÁ DISPONÍVEL
...
$produtos = Produto::getProdutos($where, 'preco_de_venda DESC', $objPaginacao->getLimit());
$resultadoProdutos;
...
<head>
    ...
    <title>Fonte das Joias | <?= $categoria->nome ?></title>
...
<body>
    ...
    <main>
        ...
        <section class="py-5 text-center justify-content-center container">
                        ...
                        <form method="get" autocomplete="off">
                            <div class="input-group input-group-sm my-3">
                                ...
                                <input name="filtroNome" type="text" class="form-control"
                                    placeholder="Pesquisar produto nesta categoria" value="<?= $filtroNome ?>">
                                <?= !strlen($filtroNome) ? '<button class="btn btn-secondary" type="submit"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-search" viewBox="0 0 16 16"><path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z" /></svg></button>' : '<a class="btn btn-secondary" role="button" href="categoria.php?id=' . $categoria->id . '">Voltar</a>'; ?>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <div class="row">
                <?= strlen($resultadoProdutos) ? $resultadoProdutos : '<div class="col-12 col-md-12"><h6 class="text-center">Nenhum produto foi cadastrado na categoria selecionada.</h6><a href="index.php" role="button" class="btn btn-warning btn-sm">Voltar</a></div><script>location.href="index.php?status=erro_produtos"</script>' ?>
            </div>
            <div class="row my-3">
                <nav>
